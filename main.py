...
COUNTRY_ORDER = ["HK", "SG", "JP", "US", "DE"]  # 输出排序
...
BANDWIDTH_MIN_MBPS = cfg.get("BANDWIDTH_MIN_MBPS", 0)  # 最低带宽要求（Mbps），0=不限
BANDWIDTH_SPEED_LIMIT = cfg.get("BANDWIDTH_SPEED_LIMIT", 0)  # 低速阈值（bytes/s），0=不启用
BANDWIDTH_SPEED_TIME = cfg.get("BANDWIDTH_SPEED_TIME", 10)  # 低速持续秒数
...
def _whois_raw(ip):
    """查询 whois，返回原始 org/netname（优先 OrgName，跳过 RIR）"""
    RIR_NAMES = {
    ...
    try:
        ...
        for line in result.stdout.split('\n'):
            m = re.match(r'(?:org-name|OrgName|owner|Owner):\s*(.+)', line, re.I)
            if m:
                cand = m.group(1).strip()
                if cand.lower() not in RIR_NAMES:
                    org_name = cand
                    break
        if not org_name:
            for line in result.stdout.split('\n'):
                m = re.match(r'(?:netname|NetName):\s*(.+)', line, re.I)
                if m:
                    net_name = m.group(1).strip()
                    break
        # org-name 为空时，从 geofeed URL 提取域名作为最终客户（优先于 netname）
        geofeed_name = None
        if not org_name:
            for line in result.stdout.split('\n'):
                m = re.match(r'geofeed:\s*https?://([^/\s]+)', line, re.I)
                if m:
                    domain_parts = m.group(1).lower().split('.')
                    if len(domain_parts) >= 2 and domain_parts[-2] not in ('com', 'net', 'org', 'co'):
                        geofeed_name = domain_parts[-2]
                    else:
                        geofeed_name = domain_parts[0] if domain_parts[0] != 'www' else domain_parts[1] if len(domain_parts) > 1 else domain_parts[0]
                    break
        name = org_name or geofeed_name or net_name or ""
        if name:
            for suffix in [', Inc.', ' Inc.', ' Inc', ', LLC', ' LLC', ', Ltd.', ' Ltd.', ' Ltd',
                           ', Limited', ' Limited', ', Co.', ' Co.', ', Corp.', ' Corp.']:
...
def _normalize_provider(name):
    ...
    # 已知厂商映射（按关键词匹配）
    if 'XTOM' in upper or 'XT OM' in upper:
        return 'xTom'
    if 'KIRINO' in upper:
        return 'Kirino'
    if 'DMIT' in upper:
        return 'DMIT'
    if 'VOLONET' in upper or 'VOLO' in upper:
        return 'Volonet'
    if 'GOMAMI' in upper or 'GO MAMI' in upper:
        return 'GoMami'
    if 'MISAKA' in upper:
        return 'Misaka'
    if 'AKARI' in upper:
    ...
    if 'BUYVM' in upper:
        return 'BuyVM'
    if 'RACKNERD' in upper:
        return 'RackNerd'
    if 'HOSTHATCH' in upper:
        return 'HostHatch'
    if 'HETZNER' in upper:
        return 'Hetzner'
    if 'OVH' in upper:
    ...
    if 'VULTR' in upper:
        return 'Vultr'
    if 'DIGITALOCEAN' in upper:
        return 'DigitalOcean'
    if 'LINODE' in upper:
        return 'Linode'
    if 'AMAZON' in upper or 'AWS' in upper:
        return 'AWS'
    if 'GOOGLE' in upper:
        return 'Google'
    if 'MICROSOFT' in upper or 'AZURE' in upper:
        return 'Azure'
    if 'CLOUDFLARE' in upper:
        return 'Cloudflare'
    if 'AKAMAI' in upper:
        return 'Akamai'

    # 去掉常见的 IP 段后缀
    name = re.sub(r'[-_]\d{1,3}[-_]\d{1,3}[-_]\d{1,3}[-_]\d{1,3}.*$', '', name)
    name = re.sub(r'[-_]\d{4,}.*$', '', name)
    name = re.sub(r'^NET[-_]', '', name)
    if not name:
        return 'Net'
...
def measure_bandwidth_curl(node_str):
    ...
    if BANDWIDTH_SPEED_LIMIT > 0:
        curl_cmd += [
            "--speed-limit", str(BANDWIDTH_SPEED_LIMIT),
            "--speed-time", str(BANDWIDTH_SPEED_TIME),
        ]
    curl_cmd += ["--insecure", BANDWIDTH_URL]
...
def write_ip_txt(final_nodes, output_file,
                 ...
                 speed_map=None, latency_map=None, provider_map=None):
    """生成 ip.txt，格式 IP:PORT#CC编号-厂商"""
    cc_counters = defaultdict(int)
    numbered_nodes = []
    for node in final_nodes:
        # 提取原始 IP:PORT 和标签
        if '#' in node:
            base, tag = node.rsplit('#', 1)
        else:
            base, tag = node, ''
        ...
        if tag:
            ...
            if len(parts) == 2 and re.match(r'^[A-Z]{2}$', parts[1]):
                # 格式: Provider-CC → 提取 CC
                prov, cc = parts
            elif re.match(r'^[A-Z]{2}$', tag):
                # 纯国家代码
                cc = tag
            # else: 非标准标签（如 MIYU），保持原样
        
        if cc:
            cc_counters[cc] += 1
            # 查询厂商
            ip = node.split(':')[0]
            whois_prov = provider_map.get(ip, '') if provider_map else ''
            display_prov = whois_prov or prov
            if display_prov:
                line = f"{base}#{cc}{cc_counters[cc]}-{display_prov}"
            else:
                line = f"{base}#{cc}{cc_counters[cc]}"
        else:
            line = node  # 固定节点保持原样
        ...
        if perline_enabled and perline_text:
            line += perline_text
        numbered_nodes.append(line)

    with open(output_file, "w", encoding="utf-8") as f:
        if header_enabled:
            for line in header_lines:
                f.write(line + "\n")
        for line in numbered_nodes:
            f.write(line + "\n")
        if footer_enabled:
...
def main():
    ...
    # 从本地 CSV 加载 IP 池
    csv_path = cfg.get("CSV_SOURCE_PATH", "")
    if csv_path and os.path.exists(csv_path):
        ...
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                country = row.get("地区", "").strip().upper()
                if ip and port:
                    csv_nodes.append(f"{ip}:{port}#{country}")
        if csv_nodes:
            seen = set()
            for n in nodes:
                seen.add(n.split('#')[0])
            ...
            for n in csv_nodes:
                ...
                if key not in seen:
                    ...
                    nodes.append(n)
                    added += 1
            print(f"从 CSV 加载：{len(csv_nodes)} 行，新增 {added} 个节点，合并后总计 {len(nodes)} 个节点。")
    ...
    if not bw_results:
        ...
        if BANDWIDTH_MIN_MBPS > 0:
            before = len(bw_results)
            bw_results = [(n, s) for n, s in bw_results if s >= BANDWIDTH_MIN_MBPS]
            after = len(bw_results)
            print(f"\n带宽过滤（≥{BANDWIDTH_MIN_MBPS} Mbps）：{before} -> {after} 个节点")
            if not bw_results:
                print(f"所有节点带宽均低于 {BANDWIDTH_MIN_MBPS} Mbps，退出。")
                sys.exit(0)
        ...
        if USE_GLOBAL_MODE:
            ...
            speed_map = {node: speed for node, speed in bw_results}
            # 按国家分组排序（COUNTRY_ORDER 优先），同国内按速度降序
            def _country_sort_key(node):
                cc = node.rsplit('#', 1)[-1] if '#' in node else '??'
                try:
                    cc_idx = COUNTRY_ORDER.index(cc)
                except ValueError:
                    cc_idx = 99
                return (cc_idx, -speed_map.get(node, 0))
            final_selected.sort(key=_country_sort_key)
        ...
        for node in final_selected:
            ...
            if ip not in seen_ips:
                seen_ips.add(ip)
                provider_map[ip] = get_provider(ip)
        for ip, prov in provider_map.items():
            if prov:
                print(f"  {ip} -> {prov}")
        ...
        for cc in COUNTRY_ORDER:
            if cc not in country_groups:
                continue
            print(f"\n  {cc}:")
            for j, node in enumerate(country_groups[cc], 1):
                speed = speed_map.get(node, 0)
                lat_sec = latency_map.get(node, float('inf'))
                ip = node.split(':')[0]
                prov = provider_map.get(ip, "")
                label = f"{node} [{prov}]" if prov else node
                if lat_sec != float('inf'):
                    print(f"    {j}. {label} 速度 {speed:.2f} Mbps 延迟 {lat_sec*1000:.2f} ms")
                else:
                    print(f"    {j}. {label} 速度 {speed:.2f} Mbps")

    # 追加固定节点
    fixed = cfg.get("FIXED_NODES", [])
    if fixed:
        for fn in fixed:
            if fn not in final_selected:
                final_selected.append(fn)
        print(f"追加固定节点：{fixed}")
    ...
    archive_path = os.path.join(script_dir, "archive", f"ip_{ts}.txt")
    os.makedirs(os.path.dirname(archive_path), exist_ok=True)
    shutil.copy2(OUTPUT_FILE, archive_path)
    print(f"留档: {archive_path}")
